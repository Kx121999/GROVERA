(() => {
  ...
  const body=document.body, cfg=window.GROVERA||{};
  ...
  // Wishlist
  const WKEY='grovera_wishlist_v1';const getWishlist=()=>{try{return JSON.parse(localStorage.getItem(WKEY)||'[]')}catch(e){return[]}};const saveWishlist=v=>{localStorage.setItem(WKEY,JSON.stringify(v));syncWishlist()};
  ...
  // Recently viewed
  const RKEY='grovera_recent_v1';const current=$('[data-current-product]');if(current){try{const p=JSON.parse(current.textContent),list=JSON.parse(localStorage.getItem(RKEY)||'[]').filter(x=>x.handle!==p.handle);list.unshift(p);localStorage.setItem(RKEY,JSON.stringify(list.slice(0,8)))}catch(e){}}
